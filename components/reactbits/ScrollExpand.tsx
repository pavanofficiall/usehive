...
// Adapted from React Bits ScrollExpand for a responsive 16:9 demo video.
...
const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  useEffect(() => {
    ...
    const measure = () => {
      const c = propsRef.current;
      stageH = c.useWindowScroll ? Math.min(window.innerHeight * 0.82, root.clientWidth * 9 / 16) : root.clientHeight;
      if (stageH <= 0) return;
